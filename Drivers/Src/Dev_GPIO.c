...
void Dev_GPIOInitialization(void)
{
	...
	for(GPIO_PInIndex = 0; GPIO_PInIndex < strlen(GPIO_INPUT_PINOUT) ; GPIO_PInIndex++ )
	{
		SIUL2.MSCR[GPIO_INPUT_PINOUT[GPIO_PInIndex]].B.IBE = 0x01;
	}

	/* Output Pins Configurations */
	for(GPIO_PInIndex = 0; GPIO_PInIndex < strlen(GPIO_OUTPUT_PINPUT) ; GPIO_PInIndex++ )
	{
